...
#include <Serial.h>
#include <ESP8266.h>
unsigned char Serial_RxData;		//定义串口接收的数据变量
unsigned char Serial_RxFlag;		//定义串口接收的标志位变量
...
{
	unsigned int Result = 1;	//设置结果初值为1
	while (Y --)			//执行Y次
	{
		Result *= X;		//将X累乘到结果
	}
...
void USART1_IRQHandler(void)
{
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)		//判断是否是USART1的接收事件触发的中断
	{
		...
		RxData[i] = Serial_RxData;
		i++;
		if((RxData[i-2] == '\r'|RxData[i-1] == '\n'))
		{
			RxData[i-2] = '\0';
			i = 0;
			ESP8266_GET();
